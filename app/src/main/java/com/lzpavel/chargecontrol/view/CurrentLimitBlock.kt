...
fun CurrentLimitBlock() {
    Row(
        ...
        var currentLimitStr by remember { mutableStateOf("${AppConfig.currentLimit}") }
        fun setLimit() {
            val lim = currentLimitStr.toIntOrNull() ?: -1
            if (lim in 0..3500000) {
                AppConfig.currentLimit = lim
            } else {
                currentLimitStr = AppConfig.currentLimit.toString()
            }
        }
        OutlinedTextField(
